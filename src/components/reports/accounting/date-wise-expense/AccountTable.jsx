  ...
  const dummyData = [
    {
      ...
      expenseHead: "Exam",
      date: "2024-01-01",
      description: "Expense from sales",
      ...
      <table className="w-full text-textGray text-center leading-6 sm:leading-8">
        <thead className="bg-bgBlue border-b-2 border-bgGray leading-10">
          <tr>
            ...
            <th className="w-2/12 min-w-32">{t("module.report.name")}</th>
            <th className="w-1/12 min-w-20">{t("module.report.amount")}</th>
            <th className="w-1/12 min-w-28">{t("module.report.expenseHead")}</th>
            <th className="w-2/12 min-w-24">{t("module.report.date")}</th>
            <th className="w-2/12 min-w-40">{t("module.report.description")}</th>
        ...
        <tbody className="divide-y font-medium">
          {dummyData.map((row, index) => (
            <tr key={index} className="text-center">
              ...
              <td className="w-1/12">{row.expenseHead}</td>
              <td className="w-2/12">{row.date}</td>
              <td className="w-2/12">{row.description}</td>
              <td className="w-1/12">{row.invoiceNo}</td>
